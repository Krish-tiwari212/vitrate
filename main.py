...
with app.app_context():
    # x = ProfData.query.filter_by(name="Kaustab Ghosh").first()
    # print(x)
    # for elem in data:
...
def all_prof(num):
    ...
    for prof in data:
        ...
        avg_rating_attend = db.session.query(func.avg(Review.attendance)).filter_by(prof_id=prof.id).scalar()
        avg_rating_marks = db.session.query(func.avg(Review.marks)).filter_by(prof_id=prof.id).scalar()
        avg_rating_research = db.session.query(func.avg(Review.research)).filter_by(prof_id=prof.id).scalar()
        ...
        try:
            ...
            # avg_rating_da = db.session.query(func.avg(Review.da)).filter_by(prof_id=prof.id).scalar()
            # avg_rating_da = round(avg_rating_da, 1)
            prof_ratings[prof.id] = {"da":avg_rating_da,"attendance":avg_rating_attend,"marks":avg_rating_marks, "re":avg_rating_research}
        except TypeError:
            pass
...
def signup():
    if request.method == "POST":
        ...
        password = (generate_password_hash(request.form.get("password"), method='pbkdf2:sha256:260000', salt_length=100))[21:]
...
def search():
    ...
    for prof in data:
        ...
        try:
            ...
            # avg_rating_da = db.session.query(func.avg(Review.da)).filter_by(prof_id=prof.id).scalar()
            # avg_rating_da = round(avg_rating_da, 1)
            prof_ratings[prof.id] = {"da": avg_rating_da, "attendance": avg_rating_attend, "marks": avg_rating_marks, "reasearch": avg_rating_research}
        except TypeError:
            pass
...
def review(num):
    if request.method == "POST":
        ...
        new_review = Review(author=current_user,attendance=request.form.get("attend"),da=request.form.get("da"),marks=request.form.get("marks"),research=request.form.get("research"),text=request.form.get('message'),prof=o)
